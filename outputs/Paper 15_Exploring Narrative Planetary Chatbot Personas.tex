% PERSONA_CATEGORY: More-than-human (planetary) persona --- non-human species/ecosystem as design persona
% PERSONA_SUBCATEGORY: Persona-as-empathy-eliciting narrative agent (first-person anthropomorphic voice of another species)
% PERSONA_SUBCATEGORY: Persona-as-interpretive design artefact for reflection (rather than authoritative representation)
% PERSONA_SUBCATEGORY: Persona-as-conversational proxy for a non-human stakeholder in design decisions
% PERSONA_SUBCATEGORY: Persona-as-credibility/transparency problem (anthropomorphic narration vs.\ factual accuracy)

\subsection*{Paper 15 --- \emph{Exploring Narrative Planetary Chatbot Personas} (Thastum, Harre \& Nielsen)}

This paper relocates the persona concept away from human user groups altogether: the
persona represents \emph{another species}. Building on Talgorn and Ullerup's notion of
\emph{planetary personas} and on Nielsen and Neuhoff's survey of non-human persona
templates, the authors treat persona as a design device for ``operationalizing other
species' perspectives'' inside otherwise anthropocentric human-centred design processes.
Crucially, the persona is not claimed to be an accurate representation of a non-human
umwelt; it is explicitly framed as an \emph{anthropomorphic, interpretive artefact} whose
value lies in provoking empathy, sense-making and ethical reflection --- ``storytelling
functions less as representation and more as a tool for critical reflection.'' This is a
different validity regime from the surrogate-user-persona papers, where fidelity to a real
population is the criterion of success.

Operationally, persona is realised as an LLM-driven chatbot conditioned by a prompt that
specifies three things: a mix of factual and empathic content, ``perform like a persona,''
and a narrative point of view. Two cases were built for deliberately
\emph{difficult-to-relate-to} species (contrasted with Li et al.'s relatable horse
chatbot): a viper persona in Claude.ai, distributed via LinkedIn and probed by a
biologist, two designers and a researcher (four conversations); and a Common Noctule bat
persona in a bespoke web prototype, evaluated by three IT students across two
design-decision scenarios (siting a solar park near a bat colony; specifying outdoor
lighting near a bat forest). Here the persona also functions as a \emph{conversational
proxy for a non-human stakeholder} consulted while making a concrete design decision ---
structurally analogous to the ``interviewable informant'' role in Paper 9, but with an
ecological rather than a market stakeholder.

The analytic focus is on the persona's \emph{performance} rather than its content: whether
it stayed in character, whether it hallucinated, and how narrative voice shaped uptake.
Narrative person is treated as an experimental variable (third- vs.\ first-person for the
bat), with first-person preferred on engagement and empathy; participants shifted to
addressing the bot as ``you,'' and some preferred the first-person version without
consciously noticing the shift. The viper conversations are coded by the interrogative
stance participants adopted (personal, factual, experimental, explorative, environmental),
showing the persona drawing on facts, embodied metaphor (Jacobson's organ, ``sssstill as
stone''), theatrical stage directions and even human myth. The paper's central problem is
therefore a persona-specific one: the very anthropomorphic narration that produces empathy
also undermines \emph{transparency and credibility} (``a bat answered my questions pretty
technical''), making persona simultaneously an empathy instrument and a trust risk ---
linking it to the risk-object framing of Paper 11, but from an empathy-design rather than
a security angle.

\begin{table*}[t]
\centering
\caption{Running taxonomy of persona conceptualizations across workshop papers (updated through Paper 15).}
...
\textbf{More-than-human (planetary) persona --- non-human species/ecosystem as design persona}; deliberately \emph{not} a representation of a user group, and validated by reflective/empathic value rather than fidelity &
